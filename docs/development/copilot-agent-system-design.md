...
- **Gateway 系（`apps/gateway/*`）はホストマシンで実行し、Docker は Agent Runtime 用に使う**
...
- セッション専用パス例: `/agent/session/<session_id>`
...
```json
{
  ...
  "session_workspace_root": "/agent/session/ses_001",
  "attachment_mount_path": "/agent/session/ses_001",
  "runtime_policy": {
    "tool_routing": {
      "mode": "gateway_only",
...
attachments:
  container_path_template: "/agent/session/{session_id}"
  decision: allow
