...
- P1〜P7（基盤/永続層/Gateway API/Approval/MCP/Agent Runtime 基盤/システム統合）は実装済み
- `apps/gateway/api` の Discord UX はモック実装として稼働済み
- `apps/agent` の Runtime は `create/resume + sendAndWait(1回)` を満たし、SDK Provider は `mock` と `copilot`（初回）を実装済み
- 次フェーズで主に残っているのは以下
  - `copilot` provider の実トークン接続テストと Discord 経路でのE2E確認
...
| P8 | Copilot SDK 実装 | `apps/agent`, `apps/gateway/mcp` | `BOT_MODE=standard` で実 SDK 実行、tool callback/permission が Gateway 経由で成立 |
...
1. `docker-compose.tcp.yml` / `docker-compose.uds.yml` を作成し、最低限 `agent` `postgres` を定義（Gateway は Compose に含めない）  
