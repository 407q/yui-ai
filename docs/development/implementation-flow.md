...
| P6 | Agent Runtime 実装 | `apps/agent`, `apps/gateway/api` | `create/resume + sendAndWait(1回)` と Bot 監督下の連携が成立 |
| P7 | 統合・復旧性（Bot主導） | 全体 | pause/resume, cancel, reboot と Bot 監視復旧をE2E確認 |
| P8 | 運用品質（Bot監視含む） | 全体 | 監査ログ、冪等、バックアップ、監視復旧運用を満たす |
...
3. 承認が必要な host 操作で Embed 承認が出る  
4. 承認/拒否/タイムアウトが結果に反映される  
5. 10分無発言で `idle_paused`、次発言で自動再開  
6. `/cancel` `/close` `/exit` `/reboot` が期待動作  
7. gateway-api 停止/agent 異常時に Bot 監視ループが復旧を試行する  
8. Discord 再送時に二重起動しない（冪等）
