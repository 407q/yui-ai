...
- migration:
  - `apps/gateway/state/migrations/0003_system_memory_bootstrap.sql`
  - `memory_entries.is_system` を追加し、`system.persona/system.policy/system.tooling` を seed
- Gateway:
  - `memory.get/search` は `system.*` 参照を許可
  - `memory.upsert/delete` は `system.*` を `memory_system_entry_read_only` で拒否
- Agent:
  - run payload に `system_memory_refs` を追加
  - `sendAndWait` 前に `memory.get` を自動実行して system memory を先読み
  - system message/runtime contract に system memory 参照義務を明記
- smoke:
  - `api:smoke` に system memory 参照可/更新拒否の検証を追加
  - `agent:smoke` に system memory preload（`memory.get`）検証を追加
