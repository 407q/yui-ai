...
| `AGENT_SOCKET_PATH` | 任意 | `${RUNTIME_SOCKET_DIR}/agent-runtime.sock`（`RUNTIME_SOCKET_DIR` 未指定時は `${XDG_RUNTIME_DIR}/yui-ai`、さらに未指定時は `/tmp/yui-ai`） | `INTERNAL_CONNECTION_MODE=uds` 時は未指定でも既定値で Agent listen socket を使用 |
...
| `RUNTIME_SOCKET_DIR` | 任意 | `${XDG_RUNTIME_DIR}/yui-ai`（`XDG_RUNTIME_DIR` 未指定時は `/tmp/yui-ai`） | Agent/Gateway UDS 共有マウント |
