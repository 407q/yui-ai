    ...
    return {
      ...
      systemMessage: {
        content: buildSystemMessageWithRuntimeContracts(
          ...
          state.workspaceRoot,
          state.routingMode,
          this.options.memoryAdminRoleId,
        ),
      },
...
function buildSystemMessageWithRuntimeContracts(
  ...
  const normalizedMemoryAdminRoleId =
    memoryAdminRoleId && memoryAdminRoleId.trim().length > 0
      ? memoryAdminRoleId.trim()
      : "unset";
  return [
    baseSystemMessage,
    "",
    ...
    `- system_memory_admin_role_id: ${normalizedMemoryAdminRoleId}`,
    routingContract,
